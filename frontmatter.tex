...
\gls{verification} of data, and is therefore a more appropriate location for this technique.
...
\item \dsiwgRef{Section}{bkm:dataVerification} has been renamed from
  ``Data Implementation'' to ``Data \Gls{verification}'', as it is a more accurate title.
\item \autoref{tab:MethodsDataProcedures} has been renamed from
  ``Mitigation methods: data implementation'' to ``Mitigation methods: data \gls{verification}''
...
lot of attention.
These are very heavily \glspl{data-driven system}, but most of the hazards that have been identified are
...
changing field.
Similarly, the \glspl{data property}
\gls{completeness}\index{Completeness!Property}, Analysability\index{Analysability Property},
...
In particular, key \index{Property!Data}\glspl{data property} that preserve safety must be actively managed.
...
\begin{itemize}
  ...
  \item A Data Safety Management Process has been developed. This is based on four phases:
	...
	\item The activities of each phase (and associated tailoring \gls{information}) are described in \autoref{bkm:activitiestailoring}.
	\item Additional guidance \gls{information} for each phase is contained in \autoref{bkm:guidance}.
