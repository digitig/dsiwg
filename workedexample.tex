...
\end{minipage}
	\begin{itemize}
		...
		\item \dsiwgTextBF{Verification Data}\index{Verification Data}: the test \glspl{dataset} to be used to support certain deployments such as integration testing and training; and
...
As with the Manufacturer, the Health Organization needs to \dsiwgTextBF{\dsiwgTextIT{identify \glspl{data artefact}s}} that are potential sources of safety hazards and understand the context of their use in the procurement / deployment lifecycles of the Health Organisation\index{Lifecycle}.
...
In later phases however, when the system functionality is specified and the system is being built, the Manufacturer will want to create a test \gls{dataset} that will be instrumental in demonstrating the correct functioning of the system. This still involves the use of \dsiwgTextBF{Verification Data}\index{Verification Data} and \dsiwgTextBF{Infrastructure Data}\index{Infrastructure Data} but there is far greater dependency on these \glspl{dataset} than the previous case. For example, if the verification or \gls{configuration data} is not sufficiently diverse or insufficiently models real-world scenarios, it is possible that erroneous and unsafe functional behaviour is present in the system during live operation despite this system having passed factory and site acceptance testing.
...
\begin{itemize}
  ...
  \item What existing barriers or mitigations\index{Mitigation} (physical, technical, procedural) exist to reduce the risk of loss of \index{Property!Data}\glspl{data property}?
  \item
...
The Manufacturer will \dsiwgTextBF{\dsiwgTextIT{Establish \glspl{dsal}\index{Assurance Level!Data}}} by considering cases where the use of specific categories of data\index{Category!Data} could give rise to \glspl{hazard}. In the first, the prototyping phase, the Manufacturer sees no use of the data that can give rise to credible clinical risk and assessing the \gls{dsal}\index{Assurance Level!Data} for that \gls{dataset} as \dsiwgTextBF{DSAL0}. 

In the second phase of the development lifecycle\index{Lifecycle!Development}, where \dsiwgTextBF{Verification Data}\index{Verification Data} and \dsiwgTextBF{Infrastructure Data}\index{Infrastructure Data} is being used to demonstrate the correct functioning of the system, the Manufacturer considers that loss of any of the \glspl{data property} of \dsiwgTextBF{\index{Integrity Property}\gls{integrity}, \index{Completeness!Property}\gls{completeness}, \index{Consistency!Property}\gls{consistency}, \index{Continuity Property}\gls{continuity}, \index{Format Property}Format, \index{Accuracy Property}\gls{accuracy}, \gls{resolution}\index{Resolution!Data}, \gls{timeliness}\index{Timeliness Property}, \index{Availability Property}\gls{availability}, \index{Fidelity/Representation Property}\gls{fidelity}, \gls{sequencing}\index{Sequencing, Data}, Intended Destination / Usage\index{Intended Destination/Usage Property}, Goldilocks\index{Goldilocks Property}, Explainability\index{Explainability Property}} of this data could give rise to \glspl{hazard}.

For example, if the verification \gls{dataset} selected is not representative of the eventual diversity experienced in practice (loss of \index{Fidelity/Representation Property}\gls{fidelity}), then it is possible that the system may contain latent software errors that could give rise to harm. However, the Manufacturer acknowledges that the system will be subject to further testing and trials in the clinical setting and so there will be other opportunities to detect errors in the system. Overall:
\begin{itemize}
...
For \dsiwgTextBF{DSAL1}\index{Assurance Level!Data} Verification Data, the tables show that the following are recommended (R) or highly recommended (HR) where loss of \glspl{data property} \dsiwgTextBF{\index{Integrity Property}\gls{integrity}, \index{Completeness!Property}\gls{completeness}, \index{Consistency!Property}\gls{consistency}, \index{Continuity Property}\gls{continuity}, \index{Format Property}Format, \index{Accuracy Property}\gls{accuracy}, \gls{resolution}{Resolution!Data}, \gls{timeliness}\index{Timeliness Property}, \index{Availability Property}\gls{availability}, \index{Fidelity/Representation Property}\gls{fidelity}, \gls{sequencing}\index{Sequencing, Data}, Intended Destination / Usage\index{Intended Destination/Usage Property}, Goldilocks\index{Goldilocks Property}, Explainability\index{Explainability Property}} can give rise to
a number of \glspl{hazard}. The extracts below indicate the mitigation\index{Mitigation} techniques identified from the tables which should form the basis of the \index{Safety Requirement!Data}\glspl{data safety requirement}. 
...
\begin{longtable*}{|C{\dsiwgColumnWidth{0.08}}|L{\dsiwgColumnWidth{0.15}}|C{\dsiwgColumnWidth{0.08}}|L{\dsiwgColumnWidth{0.69}}|}
  ...
  \hline
   TD.13 & Error Seeding & R & This is where errors are deliberately inserted into the \gls{dataset} to demonstrate the effectiveness of data validation.\\
  \hline
...
\caption{Worked Example: Derived \index{Safety Requirement!Data!Derived}\ecapitalisewords{\glsentrylong{data safety requirement}}}
  ...
  \endfirsthead
    \caption[]{Worked Example: Derived \index{Safety Requirement!Data!Derived}\ecapitalisewords{\glsentrylong{data safety requirement}} (continued)}
  \\\hline\TableHeadColourC{} & \TableHeadColour{} & \TableHeadColourCX{Guidance}\\
  \multirow{-2}*{\TableHeadColourC{Ref}} & \multirow{-2}*{\TableHeadColour{Requirement}} & \TableHeadColourCX{Reference}\\\hline
  \endhead
